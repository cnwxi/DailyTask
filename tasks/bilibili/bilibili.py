...
from tool.utils import get_configs, get_cookies
...
class Task:
    ...
    # 有改动
    def main(self):
        bilibili_cookie = get_cookies(self.config.get("cookies"))
        bili_jct = bilibili_cookie.get("bili_jct")
        coin_num = self.config.get("coin_num", 0)
        coin_type = self.config.get("coin_type", 1)
        silver2coin = self.config.get("silver2coin", False)
        ...
        if is_login:
            ...
            if coin_num > 0:
                for aid in aid_list[::-1]:
                    ...
                    if coin_num <= 0:
                        break
                coin_msg = f"今日成功投币{success_count + coins_av_count}/{self.config.get('coin_num', 5)}个"
            else:
                coin_msg = f"今日成功投币{coins_av_count}/{self.config.get('coin_num', 5)}个"
